...
#define BIM_IMG_A_AREA        46                                                //#define BIM_IMG_A_AREA        62

#define BIM_IMG_B_PAGE        6                                                 //#define BIM_IMG_B_PAGE        8
...
static void crcCheck(uint8 page, uint16 *crc)
{
  ...
  uint16 SHC_crc = crcCalcDMA(page);
...
void main(void)
{
  ...
  if ((crc[0] != 0xFFFF) && (crc[0] != 0x0000))
  {
    if (crc[0] == crc[1])
    {
      ...
      asm("LJMP 0x3030");                                                       // asm("LJMP 0x4030");
